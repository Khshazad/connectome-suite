#!/usr/bin/env python3
"""
Product 9: Natural Language AI Co-Pilot for Connectomics Generator & Module
============================================================================
Generates 9_natural_language_copilot/copilot.py and implements a Graph RAG NLP agent
for natural language query parsing, shortest path tracing, centrality analysis,
sub-circuit extraction, and neurotransmitter pathway search.
"""
...
logger = logging.getLogger("Product9_Copilot")

COPILOT_PY_CONTENT = '''#!/usr/bin/env python3
"""
...
def build_product_9(loader, output_dir: Path) -> dict:
    """Build Product 9: Natural Language AI Co-Pilot for Connectomics."""
    logger.info(f"Building Product 9 (Natural Language Co-Pilot) in {output_dir}...")
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Write copilot.py
    copilot_py_path = output_dir / "copilot.py"
    with open(copilot_py_path, "w") as f:
        f.write(COPILOT_PY_CONTENT)
    logger.info(f"✓ Wrote {copilot_py_path}")

    # Import and run copilot agent against sample queries
    sys.path.insert(0, str(output_dir))
    import copilot as copilot_module

    agent = copilot_module.ConnectomeGraphRAGCoPilot(loader.G, loader.neurons)

    test_queries = [
        "What is the shortest path between N-0 and N-42?",
        "Which Kenyon cells or motor neurons have the highest betweenness centrality in mushroom body?",
        "Show neurotransmitter pathways connecting Cholinergic neurons to GABAergic neurons",
        "Summarize the circuit structure in the antennal lobe"
    ]

    demo_session = []
    for q in test_queries:
        res = agent.ask(q)
        demo_session.append(res)

    session_path = output_dir / "copilot_demo_session.json"
    with open(session_path, "w") as f:
        json.dump(demo_session, f, indent=2)

    logger.info(f"✓ Created Product 9 artifacts in {output_dir}")
    return {
        "status": "success",
        "product_id": 9,
        "name": "Natural Language AI Co-Pilot for Connectomics",
        "demo_queries_tested": len(test_queries),
        "demo_session_path": str(session_path)
    }
...
if __name__ == "__main__":
    ...
    out_dir = Path("/mnt/data/Connectome/connectome_products/9_natural_language_copilot")
    build_product_9(loader, out_dir)
