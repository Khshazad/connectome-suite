#!/usr/bin/env python3
"""
Product 6: Automated Brain-Wide Gene-to-Circuit Receptor Mapper Generator & Module
================================================================================
Generates 6_gene_circuit_mapper/mapper.py and maps scRNA-seq receptor expression
(nAChR, GABAR, DopR, GluR) to connectome graph nodes and computes cell-type drug susceptibility.
"""
...
logger = logging.getLogger("Product6_GeneMapper")

MAPPER_PY_CONTENT = '''#!/usr/bin/env python3
"""
...
def build_product_6(loader, output_dir: Path) -> dict:
    """Build Product 6: Automated Brain-Wide Gene-to-Circuit Receptor Mapper."""
    logger.info(f"Building Product 6 (Gene-to-Circuit Mapper) in {output_dir}...")
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Write mapper.py
    mapper_py_path = output_dir / "mapper.py"
    with open(mapper_py_path, "w") as f:
        f.write(MAPPER_PY_CONTENT)
    logger.info(f"✓ Wrote {mapper_py_path}")

    # Import and run mapper against loader graph
    sys.path.insert(0, str(output_dir))
    import mapper as mapper_module

    mapper_engine = mapper_module.GeneToCircuitMapper(loader.G, loader.neurons)
    exported_files = mapper_engine.export_all(output_dir)

    return {
        "status": "success",
        "product_id": 6,
        "name": "Brain-Wide Gene-to-Circuit Receptor Mapper",
...
if __name__ == "__main__":
    ...
    out_dir = Path("/mnt/data/Connectome/connectome_products/6_gene_circuit_mapper")
    build_product_6(loader, out_dir)
